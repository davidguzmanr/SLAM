...
class FinetuningArguments(FreezeArguments, LoraArguments, RLHFArguments):
    ...
    ## 训练某些特殊层
    special_train_layers: Optional[List[int]] = field(
        default=None,
        metadata={
            "help": (
                "指定训练的某些层"
    ...
    ##只训MLP模块
    only_mlp: Optional[bool] = field(
        default=False,
        metadata={"help": "是否只训MLP模块"},
    )
    ##只训self-attention模块
    only_attention: Optional[bool] = field(
        default=False,
        metadata={"help": "是否只训self-attention模块"},
    )
    # 随机选择一样数量的层数训练
    random_layers: Optional[int] = field(
        default=None,
        metadata={"help": "随机选择相同数量的层数训练，而不是前几层"},
    )
    ## 训练Embedding层
    train_emb: Optional[bool] = field(
        default=False,
        metadata={"help": "是否训练Embedding层"},
    )
